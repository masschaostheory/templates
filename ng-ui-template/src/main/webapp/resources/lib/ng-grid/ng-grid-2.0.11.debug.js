...
angular.module('ngGrid.services').factory('$domUtilityService',['$utilityService', '$window', function($utils, $window) {
    ...
    domUtilityService.AssignGridContainers = function($scope, rootEl, grid) {
        ...
        $scope.$on('$destroy', function() {
        	...
        	if (grid.$root){//https://github.com/angular-ui/ng-grid/issues/1139
...
var ngFooter = function ($scope, grid) {
    ...
    //kyle modified this
    $scope.showCounts = grid.config.showCounts;
...
var ngSearchProvider = function ($scope, grid, $filter) {
    ...
    var searchEntireRow = function(condition, item, fieldMap, nestObjectSearch){//kyle modified this
        var result;
        for (var prop in item) {
            if (item.hasOwnProperty(prop)) {
                var c = fieldMap[prop.toLowerCase()];
                if (!c && !nestObjectSearch) {
                    continue;
                }
                var pVal = item[prop];
                if(typeof pVal === 'object' && !(pVal instanceof Date)) {
                    var objectFieldMap = convertToFieldMap(c);
                    result = searchEntireRow(condition, pVal, objectFieldMap, true);
                    if (result) {
                        return true;
                    }
                } else {
    ...
    var filterFunc = function(item) {
        for (var x = 0, len = searchConditions.length; x < len; x++) {
            ...
            var result;
            if (!condition.column) {
                result = searchEntireRow(condition, item, self.fieldMap, false);
            } else {
...
angular.module('ngGrid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('footerTemplate.html',
    ...
    "\n" +
    "            <span ng-show=\"showCounts\" class=\"ngLabel\">{{i18n.ngTotalItemsLabel}} {{maxRows()}}</span><span ng-show=\"filterText.length > 0 && showCounts\" class=\"ngLabel\">({{i18n.ngShowingItemsLabel}} {{totalFilteredItemsLength()}})</span>\r" +
    "\n" +
    "        </div>\r" +
    "\n" +
